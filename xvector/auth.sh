...
if [ $# != 3 ] ; then 
	echo "USAGE: $0 wav_path(or speaker_path) type username"
	exit 1;
fi
...
if [ -d "./data/$username/authenticate" ];then
	rm -rf ./data/$username/authenticate
fi
...
datadir=`pwd`/data/$username/authenticate
logdir=`pwd`/data/$username/authenticate/log
featdir=`pwd`/data/$username/authenticate/feat
